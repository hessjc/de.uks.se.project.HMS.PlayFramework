...
public class AssignmentControllerTest implements Constants
{
   ...
   /**
    ********************** showAssignment(lectureId, assignmentId) *************************
    */
   @Test
   public void showAssignment()
   {
      running(fakeApplication, new Runnable() {
         ...
         public void run()
         {
            ...
            Result result = testShowAssignment(ADMIN, PASSWORD);
            assertThat(status(result)).isEqualTo(OK);

            result = testShowAssignment(LECTUREADMIN, PASSWORD);
            assertThat(status(result)).isEqualTo(OK);

            result = testShowAssignment(PROOFREADER, PASSWORD);
            assertThat(status(result)).isEqualTo(OK);

            result = testShowAssignment(USER, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);

            result = testShowAssignment(STUDENT, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);

            result = testShowAssignment(STUDENTOFFIRSTLECTURE, PASSWORD);
            assertThat(status(result)).isEqualTo(OK);
         }

         private Result testShowAssignment(String email, String password)
         {
            Http.Session session = login(email, password);
            FakeRequest request = fakeRequest("get", routes.AssignmentController
                  .showAssignment(firstLecture.id, firstAssignment.id).url());
            for (Map.Entry<String, String> e : session.entrySet())
            {
               request = request.withSession(e.getKey(), e.getValue());
            }
            Result result = callAction(
                  controllers.routes.ref.AssignmentController
                        .showAssignment(firstLecture.id, firstAssignment.id), request);
            return result;
         }
      });
   }
   ...
   @Test
   public void showNotExistingAssignment()
   {
      running(fakeApplication, new Runnable() {
         @Override
         public void run()
         {
            ...
            Result result = callAction(
                  controllers.routes.ref.AssignmentController
                        .showAssignment(firstLecture.id, 2), request);
            assertThat(status(result)).isEqualTo(FORBIDDEN);
         }
      });
   }
   ...
   public void createAssignment()
   {
      running(fakeApplication, new Runnable() {
         ...
         {
            assertThat(SimpleUserServicePlugin()).isNotNull();
            Result result = testCreateAssignment(ADMIN, PASSWORD);
            assertThat(status(result)).isEqualTo(OK);

            result = testCreateAssignment(LECTUREADMIN, PASSWORD);
            assertThat(status(result)).isEqualTo(OK);

            result = testCreateAssignment(PROOFREADER, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);

            result = testCreateAssignment(USER, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);

            result = testCreateAssignment(STUDENT, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);

            result = testCreateAssignment(STUDENTOFFIRSTLECTURE, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);
         }

         private Result testCreateAssignment(String email, String password)
         {
            Http.Session session = login(email, password);
            FakeRequest request = fakeRequest("get", routes.AssignmentController
                  .createAssignment(firstLecture.id).url());
            for (Map.Entry<String, String> e : session.entrySet())
            {
               request = request.withSession(e.getKey(), e.getValue());
            }
            Result result = callAction(
                  controllers.routes.ref.AssignmentController
                        .createAssignment(firstLecture.id), request);
            return result;
         }
      });
   }
   ...
   /**
    ********************* completeAssignment(lectureId, assignmentId) ***********************
    */
   @Test
   public void completeAssignment()
   {
      running(fakeApplication, new Runnable() {
         ...
         public void run()
         {
            ...
            Result result = testCompleteAssignment(ADMIN, PASSWORD);
            assertThat(status(result)).isEqualTo(SEE_OTHER);

            result = testCompleteAssignment(LECTUREADMIN, PASSWORD);
            assertThat(status(result)).isEqualTo(SEE_OTHER);

            result = testCompleteAssignment(STUDENT, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);

            result = testCompleteAssignment(STUDENTOFFIRSTLECTURE, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);

            result = testCompleteAssignment(USER, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);

            result = testCompleteAssignment(PROOFREADER, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);
         }

         private Result testCompleteAssignment(String email, String password)
         {
            Http.Session session = login(email, password);
            FakeRequest request = fakeRequest("get", routes.AssignmentController
                  .completeAssignment(firstLecture.id, secondAssignment.id).url());
            for (Map.Entry<String, String> e : session.entrySet())
            {
               request = request.withSession(e.getKey(), e.getValue());
            }
            ...
            Result result = callAction(
                  controllers.routes.ref.AssignmentController
                        .completeAssignment(firstLecture.id, secondAssignment.id), request);

            if (status(result) == SEE_OTHER)
            {
               secondAssignment = Assignment.findAssignmentByID(secondAssignment.id);
               assertThat(secondAssignment.isCompleted()).isEqualTo(true);

               secondAssignment.setCompleted(false);
               secondAssignment.update();
               assertThat(Assignment.findAssignmentByID(secondAssignment.id).isCompleted()).isEqualTo(false);
            }
            return result;
         }
      });
   ...
   /**
    ************************ editAssignment(lectureId, assignmentId) ***********************
    */
   @Test
   public void editAssignment()
   {
      running(fakeApplication, new Runnable() {
         ...
         public void run()
         {
            ...
            Result result = testEditAssignment(ADMIN, PASSWORD);
            assertThat(status(result)).isEqualTo(OK);

            result = testEditAssignment(LECTUREADMIN, PASSWORD);
            assertThat(status(result)).isEqualTo(OK);

            result = testEditAssignment(STUDENT, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);

            result = testEditAssignment(STUDENTOFFIRSTLECTURE, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);

            result = testEditAssignment(USER, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);

            result = testEditAssignment(PROOFREADER, PASSWORD);
            assertThat(status(result)).isEqualTo(FORBIDDEN);
         }

         private Result testEditAssignment(String email, String password)
         {
            Http.Session session = login(email, password);
            FakeRequest request = fakeRequest("get", routes.AssignmentController
                  .editAssignment(firstLecture.id, secondAssignment.id).url());
            for (Map.Entry<String, String> e : session.entrySet())
            {
               request = request.withSession(e.getKey(), e.getValue());
            }
            ...
            Result result = callAction(
                  controllers.routes.ref.AssignmentController
                        .editAssignment(firstLecture.id, secondAssignment.id), request);
            return result;
         }
      });
   }
   ...
   @Test
   public void editCompletedAssignment()
   {
      running(fakeApplication, new Runnable() {
         @Override
         public void run()
         {
            ...
            Result result = testEditCompletedAssignment(ADMIN, PASSWORD);
            assertThat(status(result)).isEqualTo(SEE_OTHER);

            result = testEditCompletedAssignment(LECTUREADMIN, PASSWORD);
            assertThat(status(result)).isEqualTo(SEE_OTHER);

         }

         private Result testEditCompletedAssignment(String email, String password)
         {
            Http.Session session = login(email, password);
            FakeRequest request = fakeRequest("get", routes.AssignmentController
                  .editAssignment(firstLecture.id, firstAssignment.id).url());
            for (Map.Entry<String, String> e : session.entrySet())
            {
               request = request.withSession(e.getKey(), e.getValue());
            }
            ...
            Result result = callAction(
                  controllers.routes.ref.AssignmentController
                        .editAssignment(firstLecture.id, firstAssignment.id), request);
            return result;
         }
      });
   }
   ...
   @Test
   public void editNotExistingAssignment()
   {
      running(fakeApplication, new Runnable() {
         @Override
         public void run()
         ...
         private Result testEditCompletedAssignment(String email, String password)
         {
            Http.Session session = login(email, password);
            FakeRequest request = fakeRequest("get", routes.AssignmentController
                  .editAssignment(firstLecture.id, Assignment.findAll().size() + 1).url());
            for (Map.Entry<String, String> e : session.entrySet())
            {
               request = request.withSession(e.getKey(), e.getValue());
            }
            Result result = callAction(
                  controllers.routes.ref.AssignmentController
                        .editAssignment(firstLecture.id, Assignment.findAll().size() + 1), request);
            return result;
         }
      });
   }
   ...
   /**
    ***************** downloadStuff(lectureId, assignmentId, StuffId) ******************
    */
      @Test
      public void downloadStuff()
      {
         running(fakeApplication, new Runnable() {
            ...
            {
               assertThat(SimpleUserServicePlugin()).isNotNull();
               Result result = testdownloadStuff(ADMIN, PASSWORD);
               assertThat(status(result)).isEqualTo(OK);
   
               result = testdownloadStuff(LECTUREADMIN, PASSWORD);
               assertThat(status(result)).isEqualTo(OK);
   
               result = testdownloadStuff(PROOFREADER, PASSWORD);
               assertThat(status(result)).isEqualTo(OK);
   
               result = testdownloadStuff(USER, PASSWORD);
               assertThat(status(result)).isEqualTo(FORBIDDEN);
   
               result = testdownloadStuff(STUDENT, PASSWORD);
               assertThat(status(result)).isEqualTo(FORBIDDEN);
   
               result = testdownloadStuff(STUDENTOFFIRSTLECTURE, PASSWORD);
               assertThat(status(result)).isEqualTo(OK);
            }
   
            private Result testdownloadStuff(String email, String password)
            {
               Http.Session session = login(email, password);
               FakeRequest request = fakeRequest("get", routes.AssignmentController
                     .downloadStuff(firstLecture.id, firstAssignment.id, stuff.id).url());
               for (Map.Entry<String, String> e : session.entrySet())
               {
                  request = request.withSession(e.getKey(), e.getValue());
               }
               Result result = callAction(
                     controllers.routes.ref.AssignmentController
                           .downloadStuff(firstLecture.id, firstAssignment.id, stuff.id), request);
               return result;
            }
         });
      }
